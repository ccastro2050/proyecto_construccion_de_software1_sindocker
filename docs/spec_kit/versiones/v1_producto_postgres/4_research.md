...
(arranca en segundos, sin requisitos de RAM) y fácil de operar en
Windows: corre como SERVICIO (arranca solo) y su `psql` permite el
inicializador idempotente `crear_bd.ps1` de esta edición. Y
...
## D8 — El arranque en dos comandos desde la v1

**Alternativa descartada:** pasos manuales de pgAdmin dictados en clase.
**Decisión:** `db\crear_bd.ps1` (idempotente, con mensajes claros) +
`dotnet watch run` desde v1 — dos comandos dejan todo funcionando.
**Por qué:** el Artículo 4 de la constitución ("un solo comando") es
permanente — y la constitución gana. El entorno de v1 **crece por
