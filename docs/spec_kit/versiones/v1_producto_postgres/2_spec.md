...
- Configuración por `appsettings.json`, sobrescribible por variables de
  entorno (`ConnectionStrings__Postgres`) — la vía natural al desplegar.
- **Dos comandos** (Artículo 4): `db\crear_bd.ps1` (el inicializador
  local de la BD) + `dotnet watch run` (la API), de modo que ese par
...
1. **`.\db\crear_bd.ps1` + `dotnet watch run` — dos comandos —** dejan corriendo
...
6. **Prueba de capas:** `dotnet run --project pruebas` (o vía
   desde `api_facturas/`) ejecuta el servicio con un repositorio FALSO en
