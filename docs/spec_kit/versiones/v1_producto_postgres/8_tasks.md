...
- [ ] Tener PostgreSQL instalado (instalador oficial, servicio en 5432)
      y correr `.\db\crear_bd.ps1 -NombreBd bdfacturas_mi_v1` — crea el
      usuario del curso, SU base y los datos (idempotente) — ver
      [3_plan.md](3_plan.md) §5 y [ENTORNO_LOCAL](../../../ENTORNO_LOCAL.md).
- [ ] Crear `api_facturas/` con subcarpetas `Modelos/`, `Peticiones/`, `Controllers/`,
      `Servicios/`, `Repositorios/`, `Excepciones/` y `pruebas/`.

**Verificar:** el servicio `postgresql` corre (services.msc); un
cliente SQL a `localhost:5432` (usuario `construccion`) ve las **12 tablas**
...
## Fase 6 — El arranque completo del entorno local
- [ ] `api_facturas/Properties/launchSettings.json` con
      `applicationUrl: http://localhost:8142` (SU puerto, +100).
- [ ] `appsettings.json` con la cadena a `localhost:5432` y SU base
      (`bdfacturas_mi_v1`), usuario `construccion`.

**Verificar:** cerrar todo y repetir el arranque desde cero —
`.\db\crear_bd.ps1 -NombreBd bdfacturas_mi_v1` (dice "ya existe") y
`dotnet watch run` dejan BD y API funcionando (criterio 1); editar un
`.cs`, guardar, y verificar que recompila y reinicia solo.
