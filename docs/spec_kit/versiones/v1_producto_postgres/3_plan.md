...
```
(raíz del proyecto)
├── db/crear_bd.ps1                   # el inicializador local: usuario + BD + script
├── db/
...
por `db\crear_bd.ps1` UNA vez (crea el usuario del curso, la BD y
corre `db/bdfacturas_postgres.sql`): esta edición no necesita
...
La constitución (Artículo 4) manda: dos comandos dejan TODO
funcionando. En v1 eso es: `.\db\crear_bd.ps1` (una sola vez: crea el
usuario del curso, la BD `bdfacturas_postgres_local` y ejecuta el script
provisto — idempotente) y `dotnet watch run` (la API en el 8042, fijado
en `Properties/launchSettings.json`). La BD es el SERVICIO de PostgreSQL
de Windows (5432). El detalle del entorno está en
