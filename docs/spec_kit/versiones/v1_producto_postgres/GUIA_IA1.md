...
> `.\db\crear_bd.ps1 -NombreBd bdfacturas_mi_v1` (crear SU BD), `dotnet build` (compilar),
...
└── api_facturas/                 ← TODO el código va aquí adentro
    ...
    ├── Program.cs                ← Fase 5
    ├── Properties/launchSettings.json ← Fase 6 (su puerto 8142)
    ├── Modelos/
...
| Comandos (crear_bd.ps1, dotnet build, curl) | La terminal integrada del IDE, parado en la carpeta correcta (ver abajo) |
...
|---|---|
| `.\db\crear_bd.ps1 ...` | La **raíz** de su proyecto (ahí vive `db\`) |
| `dotnet build` · `dotnet run` · `dotnet run --project pruebas` | `api_facturas\` (ahí vive el `.csproj`) — primero `cd api_facturas` |

> Nota: en esta edición el SDK de .NET es **obligatorio** (la API corre
> con `dotnet watch run` en su máquina — no hay contenedores).
...
6. Yo trabajo en Windows con un IDE (VS Code, usando su terminal integrada
   de PowerShell), SIN Docker: PostgreSQL está instalado como servicio en
   localhost:5432 y la API corre con dotnet watch run. Dame los comandos
   para ese entorno.
7. La base de datos YA VIENE DADA en db/bdfacturas_postgres.sql y la crea
   el script provisto db\crear_bd.ps1 — no escribas ni modifiques SQL de
   creación de tablas.
8. En mi máquina TAMBIÉN corre el proyecto clonado del curso. Para que
   ambos convivan, MI proyecto:
   a. Usa el puerto 8142 para la API (fijado en
      Properties/launchSettings.json).
   b. Usa SU PROPIA base: bdfacturas_mi_v1 (creada con
      .\db\crear_bd.ps1 -NombreBd bdfacturas_mi_v1) — la cadena de
      appsettings.json apunta a localhost:5432, base bdfacturas_mi_v1,
      usuario construccion.
   Cuando me des URLs o comandos de prueba, usa localhost:8142 (API) y
   localhost:5432 con la base bdfacturas_mi_v1 (BD).
...
3. **El punto de control real es el smoke test final** (7_quickstart.md):
   ...
   `localhost:8142`, y su base es `bdfacturas_mi_v1`.
...
3. Tenga el servicio de PostgreSQL corriendo (arranca solo con Windows).
...
5. En esta máquina TAMBIÉN corre el proyecto clonado del curso. MI
   proyecto usa la API en el puerto 8142 (launchSettings) y SU PROPIA base
   bdfacturas_mi_v1 (creada con .\db\crear_bd.ps1 -NombreBd bdfacturas_mi_v1).
