...
> (el servicio de PostgreSQL corriendo y la BD creada con
> `.\db\crear_bd.ps1` — ver el [README](../README.md)). PostgreSQL
...
Llene el formulario con los datos del entorno local:
...
| Port | `5432` | El puerto del servicio local de PostgreSQL |
| Database | `bdfacturas_postgres_local` | La BD que crea el inicializador |
| Username | `construccion` | El usuario del curso (lo crea `crear_bd.ps1`) |
| Use password | `Save password` | Didáctico: credenciales de juguete |
| Password | `Construccion123!` | La del usuario del curso |
...
> El puerto es el **5432** del servicio local — en esta edición no hay
> "dos direcciones": API y SQLTools usan la misma
> (detalle en [ENTORNO_LOCAL.md](ENTORNO_LOCAL.md));
...
>
> ```
> Error opening connection Failed to connect to localhost:5432
...
SQLTools y la API de C# son solo tres clientes distintos del mismo
PostgreSQL local.
...
| 1 | Crear la conexión (localhost:5432) con test y guardarla |
